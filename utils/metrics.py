...
class MAPE(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        """Computes and returns the metric value tensor.
            ...
            Acceptable error calculation area (aeca): prevent calculation of error in distances less
            than 'minimum_y_true'. This also prevents the calculation of error where LIDAR data is
            not available.
        """
        aeca = tf.cast(y_true > self.minimum_y_true, tf.float32)
        return tf.reduce_mean(
            tf.abs(y_true - y_pred) / (y_true + self.smooth) * 100 * aeca)
...
class MSPE(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        aeca = tf.cast(y_true > self.minimum_y_true, tf.float32)
        return tf.reduce_mean(
            tf.pow(y_true - y_pred, 2) / (y_true + self.smooth) * 100 * aeca)
...
class RMSE(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        """
        aeca = tf.cast(y_true > self.minimum_y_true, tf.float32)
        return tf.sqrt(tf.reduce_mean(tf.pow(y_true - y_pred, 2) * aeca))
        ...
        return tf.sqrt(
            tf.reduce_mean(
                tf.pow(
                    ...
                    tf.math.log(y_pred + self.smooth), 2) * aeca))
...
class Log10(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        return tf.reduce_mean(
            tf.abs(
                tf.math.log(y_true + self.smooth) -
                tf.math.log(y_pred + self.smooth)) * aeca)
